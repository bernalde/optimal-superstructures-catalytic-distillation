$ontext
================================================================================
Nuevo metodo para encontrar mejores configuraciones en la columna, siguiendo el
"Algoritmo 3", como se propone en el documento de tesis 1. Para las diferentes
inicializaciones se usaron soluciones iniciales donde las variables externas
tienen los valores que se muestran a continuacion (linea 1323 de este codigo):

xvalinit(etapabup,"x1")=3;
xvalinit(etapabup,"x2")=1;
xvalinit(etapabup,"x3")=1;
xvalinit(etapabup,"x4")=2;
xvalinit(etapabup,"x5")=1;
xvalinit(etapabup,"x6")=Nboil;

Para 8 etapas internas, esta inicializacion corresponde a la que se propone en
el trabajo de Bernal. Las variables continuas se inicializarion con una solucion
factible y optima encontrada con CONOPT4.
...
set Iyc "Set de etapas cataliticas internas para la reformulacion con variables externas" /1*3/;
parameter yc_aux(Iyc,net) "Parametro auxiliar para ubicar etapas cataliticas";
...
parameter xvalinit(etapabup,extvar) "valor de inicalizacion para cada etapa de boil up";
...
***evaluacion de inicializacion factible
         ...
         gval("g2")=(xvalue(iter,"x6")-xvalue(iter,"x2"))-xvalue(iter,"x6");
         gval("g3")=-xvalue(iter,"x3");
         gval("g4")=-xvalue(iter,"x4");
         gval("g5")=1-xvalue(iter,"x5");
         gval("g6")=xvalue(iter,"x1")+xvalue(iter,"x3")+xvalue(iter,"x5")+xvalue(iter,"x4")-(xvalue(iter,"x6")-xvalue(iter,"x2"))+1;
         gval("g7")=xvalue(iter,"x6")-21;
...
***Calculo de fvalue(iter)
         ...
         elseif ord(iter) eq 1 and ord(etapabup) eq 8,
         execute_loadpoint "in_15";
         elseif ord(iter) eq 1 and ord(etapabup) eq 9,
         execute_loadpoint "in_16";
         elseif ord(iter) eq 1 and ord(etapabup) eq 10,
         execute_loadpoint "in_17";
         elseif ord(iter) eq 1 and ord(etapabup) eq 11,
         execute_loadpoint "in_18";
         elseif ord(iter) eq 1 and ord(etapabup) eq 12,
         execute_loadpoint "in_19";
         elseif ord(iter) eq 1 and ord(etapabup) eq 13,
         execute_loadpoint "in_20";
         else
         execute_loadpoint "DLR";
         );
yf(Net,F)=0;
yf(Net,'1')$(ord(net) eq floor(xvalue(iter,"x1")))=1-mod(xvalue(iter,"x1"),1);
yf(Net,'1')$(ord(net) eq 1+floor(xvalue(iter,"x1")))=mod(xvalue(iter,"x1"),1);
yf(Net,'2')$(ord(net) eq floor((xvalue(iter,"x6")-xvalue(iter,"x2"))))=1-mod((xvalue(iter,"x6")-xvalue(iter,"x2")),1);
yf(Net,'2')$(ord(net) eq 1+floor((xvalue(iter,"x6")-xvalue(iter,"x2"))))=mod((xvalue(iter,"x6")-xvalue(iter,"x2")),1);
yc(Net)=0;
yc_aux(Iyc,net)=0;
yc_aux('1',net)$(ord(Net) eq floor((xvalue(iter,"x1")+xvalue(iter,"x3"))))=1-mod((xvalue(iter,"x1")+xvalue(iter,"x3")),1);
yc_aux('1',net)$(ord(Net) eq 1+floor((xvalue(iter,"x1")+xvalue(iter,"x3"))))=mod((xvalue(iter,"x1")+xvalue(iter,"x3")),1);
yc_aux('2',net)$(ord(Net) eq floor(((xvalue(iter,"x6")-xvalue(iter,"x2"))-xvalue(iter,"x4"))))=1-mod(((xvalue(iter,"x6")-xvalue(iter,"x2"))-xvalue(iter,"x4")),1);
yc_aux('2',net)$(ord(Net) eq 1+floor(((xvalue(iter,"x6")-xvalue(iter,"x2"))-xvalue(iter,"x4"))))=mod(((xvalue(iter,"x6")-xvalue(iter,"x2"))-xvalue(iter,"x4")),1);
yc_aux('3',net)$(ord(Net) eq floor((xvalue(iter,"x1")+xvalue(iter,"x3")+xvalue(iter,"x5"))))=1-mod((xvalue(iter,"x1")+xvalue(iter,"x3")+xvalue(iter,"x5")),1);
yc_aux('3',net)$(ord(Net) eq 1+floor((xvalue(iter,"x1")+xvalue(iter,"x3")+xvalue(iter,"x5"))))=mod((xvalue(iter,"x1")+xvalue(iter,"x3")+xvalue(iter,"x5")),1);
yc(net)=sum(Iyc,yc_aux(Iyc,net));
yr(Net)$((ord(Net) ne card(Net)) and (ord(Net) ne 1))=0;
yr('2')=1;
yb(Net)$((ord(Net) ne card(Net)) and (ord(Net) ne 1))=0;
yb(Net)$(ord(Net) eq floor(xvalue(iter,"x6")))=1-mod(xvalue(iter,"x6"),1);
yb(Net)$(ord(Net) eq 1+floor(xvalue(iter,"x6")))=mod(xvalue(iter,"x6"),1);
...
***Calculo de factibilidad de variables externas para los diferentes valors d
                 ...
                 gval("g2")=((xvalue(iter,"x6")+extvarhstep("x6",signo))-(xvalue(iter,"x2")+extvarhstep("x2",signo)))-(xvalue(iter,"x6")+extvarhstep("x6",signo));
                 gval("g3")=-(xvalue(iter,"x3")+extvarhstep("x3",signo));
                 gval("g4")=-(xvalue(iter,"x4")+extvarhstep("x4",signo));
                 gval("g5")=1-(xvalue(iter,"x5")+extvarhstep("x5",signo));
                 gval("g6")=(xvalue(iter,"x1")+extvarhstep("x1",signo))+(xvalue(iter,"x3")+extvarhstep("x3",signo))+(xvalue(iter,"x5")+extvarhstep("x5",signo))+(xvalue(iter,"x4")+extvarhstep("x4",signo))-((xvalue(iter,"x6")+extvarhstep("x6",signo))-(xvalue(iter,"x2")+extvarhstep("x2",signo)))+1;
                 gval("g7")=(xvalue(iter,"x6")+extvarhstep("x6",signo))-21;
...
***Calculo de valores d cuando no hay problemas de factibilidad
                                         ...
                                         execute_loadpoint "DLR";
                                         yf(Net,F)=0;
                                         yf(Net,'1')$(ord(net) eq floor((xvalue(iter,"x1")+extvarhstep("x1",signo))))=1-mod((xvalue(iter,"x1")+extvarhstep("x1",signo)),1);
                                         yf(Net,'1')$(ord(net) eq 1+floor((xvalue(iter,"x1")+extvarhstep("x1",signo))))=mod((xvalue(iter,"x1")+extvarhstep("x1",signo)),1);
                                         yf(Net,'2')$(ord(net) eq floor(((xvalue(iter,"x6")+extvarhstep("x6",signo))-(xvalue(iter,"x2")+extvarhstep("x2",signo)))))=1-mod(((xvalue(iter,"x6")+extvarhstep("x6",signo))-(xvalue(iter,"x2")+extvarhstep("x2",signo))),1);
                                         yf(Net,'2')$(ord(net) eq 1+floor(((xvalue(iter,"x6")+extvarhstep("x6",signo))-(xvalue(iter,"x2")+extvarhstep("x2",signo)))))=mod(((xvalue(iter,"x6")+extvarhstep("x6",signo))-(xvalue(iter,"x2")+extvarhstep("x2",signo))),1);
                                         yc(Net)=0;
                                         yc_aux(Iyc,net)=0;
                                         yc_aux('1',net)$(ord(Net) eq floor(((xvalue(iter,"x1")+extvarhstep("x1",signo))+(xvalue(iter,"x3")+extvarhstep("x3",signo)))))=1-mod(((xvalue(iter,"x1")+extvarhstep("x1",signo))+(xvalue(iter,"x3")+extvarhstep("x3",signo))),1);
                                         yc_aux('1',net)$(ord(Net) eq 1+floor(((xvalue(iter,"x1")+extvarhstep("x1",signo))+(xvalue(iter,"x3")+extvarhstep("x3",signo)))))=mod(((xvalue(iter,"x1")+extvarhstep("x1",signo))+(xvalue(iter,"x3")+extvarhstep("x3",signo))),1);
                                         yc_aux('2',net)$(ord(Net) eq floor((((xvalue(iter,"x6")+extvarhstep("x6",signo))-(xvalue(iter,"x2")+extvarhstep("x2",signo)))-(xvalue(iter,"x4")+extvarhstep("x4",signo)))))=1-mod((((xvalue(iter,"x6")+extvarhstep("x6",signo))-(xvalue(iter,"x2")+extvarhstep("x2",signo)))-(xvalue(iter,"x4")+extvarhstep("x4",signo))),1);
                                         yc_aux('2',net)$(ord(Net) eq 1+floor((((xvalue(iter,"x6")+extvarhstep("x6",signo))-(xvalue(iter,"x2")+extvarhstep("x2",signo)))-(xvalue(iter,"x4")+extvarhstep("x4",signo)))))=mod((((xvalue(iter,"x6")+extvarhstep("x6",signo))-(xvalue(iter,"x2")+extvarhstep("x2",signo)))-(xvalue(iter,"x4")+extvarhstep("x4",signo))),1);
                                         yc_aux('3',net)$(ord(Net) eq floor(((xvalue(iter,"x1")+extvarhstep("x1",signo))+(xvalue(iter,"x3")+extvarhstep("x3",signo))+(xvalue(iter,"x5")+extvarhstep("x5",signo)))))=1-mod(((xvalue(iter,"x1")+extvarhstep("x1",signo))+(xvalue(iter,"x3")+extvarhstep("x3",signo))+(xvalue(iter,"x5")+extvarhstep("x5",signo))),1);
                                         yc_aux('3',net)$(ord(Net) eq 1+floor(((xvalue(iter,"x1")+extvarhstep("x1",signo))+(xvalue(iter,"x3")+extvarhstep("x3",signo))+(xvalue(iter,"x5")+extvarhstep("x5",signo)))))=mod(((xvalue(iter,"x1")+extvarhstep("x1",signo))+(xvalue(iter,"x3")+extvarhstep("x3",signo))+(xvalue(iter,"x5")+extvarhstep("x5",signo))),1);
                                         yc(net)=sum(Iyc,yc_aux(Iyc,net));
                                         yr(Net)$((ord(Net) ne card(Net)) and (ord(Net) ne 1))=0;
                                         yr('2')=1;
                                         yb(Net)$((ord(Net) ne card(Net)) and (ord(Net) ne 1))=0;
                                         yb(Net)$(ord(Net) eq floor((xvalue(iter,"x6")+extvarhstep("x6",signo))))=1-mod((xvalue(iter,"x6")+extvarhstep("x6",signo)),1);
                                         yb(Net)$(ord(Net) eq 1+floor((xvalue(iter,"x6")+extvarhstep("x6",signo))))=mod((xvalue(iter,"x6")+extvarhstep("x6",signo)),1);
                                         par(Net)$((ord(Net) ne card(Net)) and (ord(Net) ne 1))=(sum(Net1$((ord(Net1) ge 2) and (ord(Net1) le ord(Net))),yr(Net1)))+(yb(Net))-(sum(Net1$((ord(Net1) ge 2) and (ord(Net1) le ord(Net))),yb(Net1)));
                                         ye(Net)$((ord(Net) ne card(Net)) and (ord(Net) ne 1))=par(Net)*(1-yc(Net))*CASE+par(Net)*(1-CASE);
                                         solve Rx_OCP_index1 using nlp minimizing zobj;
                                         mstatS4(iter,extvar,signo,iner1)=Rx_OCP_index1.modelstat;
...
***Calculo de valores d cuando hay problemas de factibilidad
                                         ...
                                         yf(Net,F)=0;
                                         yf(Net,'1')$(ord(net) eq floor(xvalue(iter,"x1")+((extvarhstep("x1",signo))/(hstep))*(hiner1(iner1))))=1-mod(xvalue(iter,"x1")+((extvarhstep("x1",signo))/(hstep))*(hiner1(iner1)),1);
                                         yf(Net,'1')$(ord(net) eq 1+floor(xvalue(iter,"x1")+((extvarhstep("x1",signo))/(hstep))*(hiner1(iner1))))=mod(xvalue(iter,"x1")+((extvarhstep("x1",signo))/(hstep))*(hiner1(iner1)),1);
                                         yf(Net,'2')$(ord(net) eq floor((xvalue(iter,"x6")+((extvarhstep("x6",signo))/(hstep))*(hiner1(iner1)))-(xvalue(iter,"x2")+((extvarhstep("x2",signo))/(hstep))*(hiner1(iner1)))))=1-mod((xvalue(iter,"x6")+((extvarhstep("x6",signo))/(hstep))*(hiner1(iner1)))-(xvalue(iter,"x2")+((extvarhstep("x2",signo))/(hstep))*(hiner1(iner1))),1);
                                         yf(Net,'2')$(ord(net) eq 1+floor((xvalue(iter,"x6")+((extvarhstep("x6",signo))/(hstep))*(hiner1(iner1)))-(xvalue(iter,"x2")+((extvarhstep("x2",signo))/(hstep))*(hiner1(iner1)))))=mod((xvalue(iter,"x6")+((extvarhstep("x6",signo))/(hstep))*(hiner1(iner1)))-(xvalue(iter,"x2")+((extvarhstep("x2",signo))/(hstep))*(hiner1(iner1))),1);
                                         yc(Net)=0;
                                         yc_aux(Iyc,net)=0;
                                         yc_aux('1',net)$(ord(Net) eq floor((xvalue(iter,"x1")+((extvarhstep("x1",signo))/(hstep))*(hiner1(iner1)))+(xvalue(iter,"x3")+((extvarhstep("x3",signo))/(hstep))*(hiner1(iner1)))))=1-mod((xvalue(iter,"x1")+((extvarhstep("x1",signo))/(hstep))*(hiner1(iner1)))+(xvalue(iter,"x3")+((extvarhstep("x3",signo))/(hstep))*(hiner1(iner1))),1);
                                         yc_aux('1',net)$(ord(Net) eq 1+floor((xvalue(iter,"x1")+((extvarhstep("x1",signo))/(hstep))*(hiner1(iner1)))+(xvalue(iter,"x3")+((extvarhstep("x3",signo))/(hstep))*(hiner1(iner1)))))=mod((xvalue(iter,"x1")+((extvarhstep("x1",signo))/(hstep))*(hiner1(iner1)))+(xvalue(iter,"x3")+((extvarhstep("x3",signo))/(hstep))*(hiner1(iner1))),1);
                                         yc_aux('2',net)$(ord(Net) eq floor(((xvalue(iter,"x6")+((extvarhstep("x6",signo))/(hstep))*(hiner1(iner1)))-(xvalue(iter,"x2")+((extvarhstep("x2",signo))/(hstep))*(hiner1(iner1))))-(xvalue(iter,"x4")+((extvarhstep("x4",signo))/(hstep))*(hiner1(iner1)))))=1-mod(((xvalue(iter,"x6")+((extvarhstep("x6",signo))/(hstep))*(hiner1(iner1)))-(xvalue(iter,"x2")+((extvarhstep("x2",signo))/(hstep))*(hiner1(iner1))))-(xvalue(iter,"x4")+((extvarhstep("x4",signo))/(hstep))*(hiner1(iner1))),1);
                                         yc_aux('2',net)$(ord(Net) eq 1+floor(((xvalue(iter,"x6")+((extvarhstep("x6",signo))/(hstep))*(hiner1(iner1)))-(xvalue(iter,"x2")+((extvarhstep("x2",signo))/(hstep))*(hiner1(iner1))))-(xvalue(iter,"x4")+((extvarhstep("x4",signo))/(hstep))*(hiner1(iner1)))))=mod(((xvalue(iter,"x6")+((extvarhstep("x6",signo))/(hstep))*(hiner1(iner1)))-(xvalue(iter,"x2")+((extvarhstep("x2",signo))/(hstep))*(hiner1(iner1))))-(xvalue(iter,"x4")+((extvarhstep("x4",signo))/(hstep))*(hiner1(iner1))),1);
                                         yc_aux('3',net)$(ord(Net) eq floor((xvalue(iter,"x1")+((extvarhstep("x1",signo))/(hstep))*(hiner1(iner1)))+(xvalue(iter,"x3")+((extvarhstep("x3",signo))/(hstep))*(hiner1(iner1)))+(xvalue(iter,"x5")+((extvarhstep("x5",signo))/(hstep))*(hiner1(iner1)))))=1-mod((xvalue(iter,"x1")+((extvarhstep("x1",signo))/(hstep))*(hiner1(iner1)))+(xvalue(iter,"x3")+((extvarhstep("x3",signo))/(hstep))*(hiner1(iner1)))+(xvalue(iter,"x5")+((extvarhstep("x5",signo))/(hstep))*(hiner1(iner1))),1);
                                         yc_aux('3',net)$(ord(Net) eq 1+floor((xvalue(iter,"x1")+((extvarhstep("x1",signo))/(hstep))*(hiner1(iner1)))+(xvalue(iter,"x3")+((extvarhstep("x3",signo))/(hstep))*(hiner1(iner1)))+(xvalue(iter,"x5")+((extvarhstep("x5",signo))/(hstep))*(hiner1(iner1)))))=mod((xvalue(iter,"x1")+((extvarhstep("x1",signo))/(hstep))*(hiner1(iner1)))+(xvalue(iter,"x3")+((extvarhstep("x3",signo))/(hstep))*(hiner1(iner1)))+(xvalue(iter,"x5")+((extvarhstep("x5",signo))/(hstep))*(hiner1(iner1))),1);
                                         yc(net)=sum(Iyc,yc_aux(Iyc,net));
                                         yr(Net)$((ord(Net) ne card(Net)) and (ord(Net) ne 1))=0;
                                         yr('2')=1;
                                         yb(Net)$((ord(Net) ne card(Net)) and (ord(Net) ne 1))=0;
                                         yb(Net)$(ord(net) eq floor(xvalue(iter,"x6")+((extvarhstep("x6",signo))/(hstep))*(hiner1(iner1))))=1-mod(xvalue(iter,"x6")+((extvarhstep("x6",signo))/(hstep))*(hiner1(iner1)),1);
                                         yb(Net)$(ord(net) eq 1+floor(xvalue(iter,"x6")+((extvarhstep("x6",signo))/(hstep))*(hiner1(iner1))))=mod(xvalue(iter,"x6")+((extvarhstep("x6",signo))/(hstep))*(hiner1(iner1)),1);
                                         par(Net)$((ord(Net) ne card(Net)) and (ord(Net) ne 1))=(sum(Net1$((ord(Net1) ge 2) and (ord(Net1) le ord(Net))),yr(Net1)))+(yb(Net))-(sum(Net1$((ord(Net1) ge 2) and (ord(Net1) le ord(Net))),yb(Net1)));
                                         ye(Net)$((ord(Net) ne card(Net)) and (ord(Net) ne 1))=par(Net)*(1-yc(Net))*CASE+par(Net)*(1-CASE);
                                         solve Rx_OCP_index1 using nlp minimizing zobj;
                                         mstatS5(iter,extvar,signo,iner1)=Rx_OCP_index1.modelstat;
                                         execute_unload "DLR1";
...
stop3=0;
         ...
         gval("g2")=((xvalue(iter,"x6")+(selectd(iter,"x6","mas")-selectd(iter,"x6","menos"))*hiner2(iner2))-(xvalue(iter,"x2")+(selectd(iter,"x2","mas")-selectd(iter,"x2","menos"))*hiner2(iner2)))-(xvalue(iter,"x6")+(selectd(iter,"x6","mas")-selectd(iter,"x6","menos"))*hiner2(iner2));
         gval("g3")=-(xvalue(iter,"x3")+(selectd(iter,"x3","mas")-selectd(iter,"x3","menos"))*hiner2(iner2));
         gval("g4")=-(xvalue(iter,"x4")+(selectd(iter,"x4","mas")-selectd(iter,"x4","menos"))*hiner2(iner2));
         gval("g5")=1-(xvalue(iter,"x5")+(selectd(iter,"x5","mas")-selectd(iter,"x5","menos"))*hiner2(iner2));
         gval("g6")=(xvalue(iter,"x1")+(selectd(iter,"x1","mas")-selectd(iter,"x1","menos"))*hiner2(iner2))+(xvalue(iter,"x3")+(selectd(iter,"x3","mas")-selectd(iter,"x3","menos"))*hiner2(iner2))+(xvalue(iter,"x5")+(selectd(iter,"x5","mas")-selectd(iter,"x5","menos"))*hiner2(iner2))+(xvalue(iter,"x4")+(selectd(iter,"x4","mas")-selectd(iter,"x4","menos"))*hiner2(iner2))-((xvalue(iter,"x6")+(selectd(iter,"x6","mas")-selectd(iter,"x6","menos"))*hiner2(iner2))-(xvalue(iter,"x2")+(selectd(iter,"x2","mas")-selectd(iter,"x2","menos"))*hiner2(iner2)))+1;
         gval("g7")=(xvalue(iter,"x6")+(selectd(iter,"x6","mas")-selectd(iter,"x6","menos"))*hiner2(iner2))-21;
                 if( sum(extineq$(gval(extineq) gt 0 ),gval(extineq)) gt 0  ,
                 stop4=1;
                 stop3=1;
                 );
...
***minimizacion en la direccion seleccionada cuando no hay problemas de convergencia
                           yf(Net,F)=0;
                           yf(Net,'1')$(ord(net) eq floor((xvalue(iter,"x1")+(selectd(iter,"x1","mas")-selectd(iter,"x1","menos"))*hiner2(iner2))))=1-mod((xvalue(iter,"x1")+(selectd(iter,"x1","mas")-selectd(iter,"x1","menos"))*hiner2(iner2)),1);
                           yf(Net,'1')$(ord(net) eq 1+floor((xvalue(iter,"x1")+(selectd(iter,"x1","mas")-selectd(iter,"x1","menos"))*hiner2(iner2))))=mod((xvalue(iter,"x1")+(selectd(iter,"x1","mas")-selectd(iter,"x1","menos"))*hiner2(iner2)),1);
                           yf(Net,'2')$(ord(net) eq floor(((xvalue(iter,"x6")+(selectd(iter,"x6","mas")-selectd(iter,"x6","menos"))*hiner2(iner2))-(xvalue(iter,"x2")+(selectd(iter,"x2","mas")-selectd(iter,"x2","menos"))*hiner2(iner2)))))=1-mod(((xvalue(iter,"x6")+(selectd(iter,"x6","mas")-selectd(iter,"x6","menos"))*hiner2(iner2))-(xvalue(iter,"x2")+(selectd(iter,"x2","mas")-selectd(iter,"x2","menos"))*hiner2(iner2))),1);
                           yf(Net,'2')$(ord(net) eq 1+floor(((xvalue(iter,"x6")+(selectd(iter,"x6","mas")-selectd(iter,"x6","menos"))*hiner2(iner2))-(xvalue(iter,"x2")+(selectd(iter,"x2","mas")-selectd(iter,"x2","menos"))*hiner2(iner2)))))=mod(((xvalue(iter,"x6")+(selectd(iter,"x6","mas")-selectd(iter,"x6","menos"))*hiner2(iner2))-(xvalue(iter,"x2")+(selectd(iter,"x2","mas")-selectd(iter,"x2","menos"))*hiner2(iner2))),1);
                           yc(Net)=0;
                           yc_aux(Iyc,net)=0;
                           yc_aux('1',net)$(ord(Net) eq floor(((xvalue(iter,"x1")+(selectd(iter,"x1","mas")-selectd(iter,"x1","menos"))*hiner2(iner2))+(xvalue(iter,"x3")+(selectd(iter,"x3","mas")-selectd(iter,"x3","menos"))*hiner2(iner2)))))=1-mod(((xvalue(iter,"x1")+(selectd(iter,"x1","mas")-selectd(iter,"x1","menos"))*hiner2(iner2))+(xvalue(iter,"x3")+(selectd(iter,"x3","mas")-selectd(iter,"x3","menos"))*hiner2(iner2))),1);
                           yc_aux('1',net)$(ord(Net) eq 1+floor(((xvalue(iter,"x1")+(selectd(iter,"x1","mas")-selectd(iter,"x1","menos"))*hiner2(iner2))+(xvalue(iter,"x3")+(selectd(iter,"x3","mas")-selectd(iter,"x3","menos"))*hiner2(iner2)))))=mod(((xvalue(iter,"x1")+(selectd(iter,"x1","mas")-selectd(iter,"x1","menos"))*hiner2(iner2))+(xvalue(iter,"x3")+(selectd(iter,"x3","mas")-selectd(iter,"x3","menos"))*hiner2(iner2))),1);
                           yc_aux('2',net)$(ord(Net) eq floor((((xvalue(iter,"x6")+(selectd(iter,"x6","mas")-selectd(iter,"x6","menos"))*hiner2(iner2))-(xvalue(iter,"x2")+(selectd(iter,"x2","mas")-selectd(iter,"x2","menos"))*hiner2(iner2)))-(xvalue(iter,"x4")+(selectd(iter,"x4","mas")-selectd(iter,"x4","menos"))*hiner2(iner2)))))=1-mod((((xvalue(iter,"x6")+(selectd(iter,"x6","mas")-selectd(iter,"x6","menos"))*hiner2(iner2))-(xvalue(iter,"x2")+(selectd(iter,"x2","mas")-selectd(iter,"x2","menos"))*hiner2(iner2)))-(xvalue(iter,"x4")+(selectd(iter,"x4","mas")-selectd(iter,"x4","menos"))*hiner2(iner2))),1);
                           yc_aux('2',net)$(ord(Net) eq 1+floor((((xvalue(iter,"x6")+(selectd(iter,"x6","mas")-selectd(iter,"x6","menos"))*hiner2(iner2))-(xvalue(iter,"x2")+(selectd(iter,"x2","mas")-selectd(iter,"x2","menos"))*hiner2(iner2)))-(xvalue(iter,"x4")+(selectd(iter,"x4","mas")-selectd(iter,"x4","menos"))*hiner2(iner2)))))=mod((((xvalue(iter,"x6")+(selectd(iter,"x6","mas")-selectd(iter,"x6","menos"))*hiner2(iner2))-(xvalue(iter,"x2")+(selectd(iter,"x2","mas")-selectd(iter,"x2","menos"))*hiner2(iner2)))-(xvalue(iter,"x4")+(selectd(iter,"x4","mas")-selectd(iter,"x4","menos"))*hiner2(iner2))),1);
                           yc_aux('3',net)$(ord(Net) eq floor(((xvalue(iter,"x1")+(selectd(iter,"x1","mas")-selectd(iter,"x1","menos"))*hiner2(iner2))+(xvalue(iter,"x3")+(selectd(iter,"x3","mas")-selectd(iter,"x3","menos"))*hiner2(iner2))+(xvalue(iter,"x5")+(selectd(iter,"x5","mas")-selectd(iter,"x5","menos"))*hiner2(iner2)))))=1-mod(((xvalue(iter,"x1")+(selectd(iter,"x1","mas")-selectd(iter,"x1","menos"))*hiner2(iner2))+(xvalue(iter,"x3")+(selectd(iter,"x3","mas")-selectd(iter,"x3","menos"))*hiner2(iner2))+(xvalue(iter,"x5")+(selectd(iter,"x5","mas")-selectd(iter,"x5","menos"))*hiner2(iner2))),1);
                           yc_aux('3',net)$(ord(Net) eq 1+floor(((xvalue(iter,"x1")+(selectd(iter,"x1","mas")-selectd(iter,"x1","menos"))*hiner2(iner2))+(xvalue(iter,"x3")+(selectd(iter,"x3","mas")-selectd(iter,"x3","menos"))*hiner2(iner2))+(xvalue(iter,"x5")+(selectd(iter,"x5","mas")-selectd(iter,"x5","menos"))*hiner2(iner2)))))=mod(((xvalue(iter,"x1")+(selectd(iter,"x1","mas")-selectd(iter,"x1","menos"))*hiner2(iner2))+(xvalue(iter,"x3")+(selectd(iter,"x3","mas")-selectd(iter,"x3","menos"))*hiner2(iner2))+(xvalue(iter,"x5")+(selectd(iter,"x5","mas")-selectd(iter,"x5","menos"))*hiner2(iner2))),1);
                           yc(net)=sum(Iyc,yc_aux(Iyc,net));
                           yr(Net)$((ord(Net) ne card(Net)) and (ord(Net) ne 1))=0;
                           yr('2')=1;
                           yb(Net)$((ord(Net) ne card(Net)) and (ord(Net) ne 1))=0;
                           yb(Net)$(ord(Net) eq floor((xvalue(iter,"x6")+(selectd(iter,"x6","mas")-selectd(iter,"x6","menos"))*hiner2(iner2))))=1-mod((xvalue(iter,"x6")+(selectd(iter,"x6","mas")-selectd(iter,"x6","menos"))*hiner2(iner2)),1);
                           yb(Net)$(ord(Net) eq 1+floor((xvalue(iter,"x6")+(selectd(iter,"x6","mas")-selectd(iter,"x6","menos"))*hiner2(iner2))))=mod((xvalue(iter,"x6")+(selectd(iter,"x6","mas")-selectd(iter,"x6","menos"))*hiner2(iner2)),1);
                           par(Net)$((ord(Net) ne card(Net)) and (ord(Net) ne 1))=(sum(Net1$((ord(Net1) ge 2) and (ord(Net1) le ord(Net))),yr(Net1)))+(yb(Net))-(sum(Net1$((ord(Net1) ge 2) and (ord(Net1) le ord(Net))),yb(Net1)));
                           ye(Net)$((ord(Net) ne card(Net)) and (ord(Net) ne 1))=par(Net)*(1-yc(Net))*CASE+par(Net)*(1-CASE);
                           solve Rx_OCP_index1 using nlp minimizing zobj;
...
***minimizacion en la direccion seleccionada cuando hay problemas de convergencia
                           ...
                           yf(Net,F)=0;
                           yf(Net,'1')$(ord(net) eq floor(xvalue(iter,"x1")+(selectd(iter,"x1","mas")-selectd(iter,"x1","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep)))=1-mod(xvalue(iter,"x1")+(selectd(iter,"x1","mas")-selectd(iter,"x1","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep),1);
                           yf(Net,'1')$(ord(net) eq 1+floor(xvalue(iter,"x1")+(selectd(iter,"x1","mas")-selectd(iter,"x1","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep)))=mod(xvalue(iter,"x1")+(selectd(iter,"x1","mas")-selectd(iter,"x1","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep),1);
                           yf(Net,'2')$(ord(net) eq floor((xvalue(iter,"x6")+(selectd(iter,"x6","mas")-selectd(iter,"x6","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep))-(xvalue(iter,"x2")+(selectd(iter,"x2","mas")-selectd(iter,"x2","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep))))=1-mod((xvalue(iter,"x6")+(selectd(iter,"x6","mas")-selectd(iter,"x6","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep))-(xvalue(iter,"x2")+(selectd(iter,"x2","mas")-selectd(iter,"x2","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep)),1);
                           yf(Net,'2')$(ord(net) eq 1+floor((xvalue(iter,"x6")+(selectd(iter,"x6","mas")-selectd(iter,"x6","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep))-(xvalue(iter,"x2")+(selectd(iter,"x2","mas")-selectd(iter,"x2","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep))))=mod((xvalue(iter,"x6")+(selectd(iter,"x6","mas")-selectd(iter,"x6","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep))-(xvalue(iter,"x2")+(selectd(iter,"x2","mas")-selectd(iter,"x2","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep)),1);
                           yc(Net)=0;
                           yc_aux(Iyc,net)=0;
                           yc_aux('1',net)$(ord(Net) eq floor((xvalue(iter,"x1")+(selectd(iter,"x1","mas")-selectd(iter,"x1","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep))+(xvalue(iter,"x3")+(selectd(iter,"x3","mas")-selectd(iter,"x3","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep))))=1-mod((xvalue(iter,"x1")+(selectd(iter,"x1","mas")-selectd(iter,"x1","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep))+(xvalue(iter,"x3")+(selectd(iter,"x3","mas")-selectd(iter,"x3","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep)),1);
                           yc_aux('1',net)$(ord(Net) eq 1+floor((xvalue(iter,"x1")+(selectd(iter,"x1","mas")-selectd(iter,"x1","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep))+(xvalue(iter,"x3")+(selectd(iter,"x3","mas")-selectd(iter,"x3","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep))))=mod((xvalue(iter,"x1")+(selectd(iter,"x1","mas")-selectd(iter,"x1","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep))+(xvalue(iter,"x3")+(selectd(iter,"x3","mas")-selectd(iter,"x3","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep)),1);
                           yc_aux('2',net)$(ord(Net) eq floor(((xvalue(iter,"x6")+(selectd(iter,"x6","mas")-selectd(iter,"x6","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep))-(xvalue(iter,"x2")+(selectd(iter,"x2","mas")-selectd(iter,"x2","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep)))-(xvalue(iter,"x4")+(selectd(iter,"x4","mas")-selectd(iter,"x4","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep))))=1-mod(((xvalue(iter,"x6")+(selectd(iter,"x6","mas")-selectd(iter,"x6","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep))-(xvalue(iter,"x2")+(selectd(iter,"x2","mas")-selectd(iter,"x2","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep)))-(xvalue(iter,"x4")+(selectd(iter,"x4","mas")-selectd(iter,"x4","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep)),1);
                           yc_aux('2',net)$(ord(Net) eq 1+floor(((xvalue(iter,"x6")+(selectd(iter,"x6","mas")-selectd(iter,"x6","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep))-(xvalue(iter,"x2")+(selectd(iter,"x2","mas")-selectd(iter,"x2","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep)))-(xvalue(iter,"x4")+(selectd(iter,"x4","mas")-selectd(iter,"x4","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep))))=mod(((xvalue(iter,"x6")+(selectd(iter,"x6","mas")-selectd(iter,"x6","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep))-(xvalue(iter,"x2")+(selectd(iter,"x2","mas")-selectd(iter,"x2","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep)))-(xvalue(iter,"x4")+(selectd(iter,"x4","mas")-selectd(iter,"x4","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep)),1);
                           yc_aux('3',net)$(ord(Net) eq floor((xvalue(iter,"x1")+(selectd(iter,"x1","mas")-selectd(iter,"x1","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep))+(xvalue(iter,"x3")+(selectd(iter,"x3","mas")-selectd(iter,"x3","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep))+(xvalue(iter,"x5")+(selectd(iter,"x5","mas")-selectd(iter,"x5","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep))))=1-mod((xvalue(iter,"x1")+(selectd(iter,"x1","mas")-selectd(iter,"x1","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep))+(xvalue(iter,"x3")+(selectd(iter,"x3","mas")-selectd(iter,"x3","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep))+(xvalue(iter,"x5")+(selectd(iter,"x5","mas")-selectd(iter,"x5","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep)),1);
                           yc_aux('3',net)$(ord(Net) eq 1+floor((xvalue(iter,"x1")+(selectd(iter,"x1","mas")-selectd(iter,"x1","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep))+(xvalue(iter,"x3")+(selectd(iter,"x3","mas")-selectd(iter,"x3","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep))+(xvalue(iter,"x5")+(selectd(iter,"x5","mas")-selectd(iter,"x5","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep))))=mod((xvalue(iter,"x1")+(selectd(iter,"x1","mas")-selectd(iter,"x1","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep))+(xvalue(iter,"x3")+(selectd(iter,"x3","mas")-selectd(iter,"x3","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep))+(xvalue(iter,"x5")+(selectd(iter,"x5","mas")-selectd(iter,"x5","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep)),1);
                           yc(net)=sum(Iyc,yc_aux(Iyc,net));
                           yr(Net)$((ord(Net) ne card(Net)) and (ord(Net) ne 1))=0;
                           yr('2')=1;
                           yb(Net)$((ord(Net) ne card(Net)) and (ord(Net) ne 1))=0;
                           yb(Net)$(ord(net) eq floor(xvalue(iter,"x6")+(selectd(iter,"x6","mas")-selectd(iter,"x6","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep)))=1-mod(xvalue(iter,"x6")+(selectd(iter,"x6","mas")-selectd(iter,"x6","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep),1);
                           yb(Net)$(ord(net) eq 1+floor(xvalue(iter,"x6")+(selectd(iter,"x6","mas")-selectd(iter,"x6","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep)))=mod(xvalue(iter,"x6")+(selectd(iter,"x6","mas")-selectd(iter,"x6","menos"))*(hiner2(iner2)+hiner1(iner1)-hstep),1);
                           par(Net)$((ord(Net) ne card(Net)) and (ord(Net) ne 1))=(sum(Net1$((ord(Net1) ge 2) and (ord(Net1) le ord(Net))),yr(Net1)))+(yb(Net))-(sum(Net1$((ord(Net1) ge 2) and (ord(Net1) le ord(Net))),yb(Net1)));
                           ye(Net)$((ord(Net) ne card(Net)) and (ord(Net) ne 1))=par(Net)*(1-yc(Net))*CASE+par(Net)*(1-CASE);
                           solve Rx_OCP_index1 using nlp minimizing zobj;
...
execute_unload "ALG_SEPARABLE_INFO",objval,CPUtime;
